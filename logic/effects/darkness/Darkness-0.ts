...
export const execute = (card: PlayedCard, laneIndex: number, state: GameState, context: EffectContext): EffectResult => {
    ...
    let newState = drawForPlayer(state, cardOwner, 3);
    newState = log(newState, cardOwner, "Darkness-0: Draw 3 cards.");
    ...
    if (opponentCoveredCards.length > 0) {
        newState = log(newState, cardOwner, "Darkness-0: Prompts to shift 1 of opponent's covered cards.");
