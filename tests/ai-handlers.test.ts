...
describe('AI Handler Tests - Generic Handler Migration', () => {
    ...
    describe('Prompts that lead to generic handlers', () => {
        it('Easy AI handles prompt_death_1_effect', () => {
            const sourceCard = createCard('Death', 1, true);
            state.opponent.lanes[0] = [sourceCard];
            state = recalculateAllLaneValues(state);
            ...
            const action = easyAI(state, state.actionRequired);
            expect(action.type).toBe('resolveDeath1Prompt');
        });

        it('Normal AI handles prompt_shift_or_flip_for_light_2', () => {
            const revealedCard = createCard('Fire', 2, false);
            const sourceCard = createCard('Light', 2, true);
            ...
            const action = normalAI(state, state.actionRequired);
            expect(action.type).toBe('resolveLight2Prompt');
            expect(['shift', 'flip', 'skip']).toContain(action.choice);
        });
    });
});
